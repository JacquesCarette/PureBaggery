...
The price we pay is that $\ElF{\VERTPIF{\V{N}}{\V{T}}}$ is \emph{not} the same type as $\VERTPI{\V{N}}{\LAM{n}{\ElF{\V{T}\:\V{n}}}}$ when $\V{N}$ and $\V{T}$ are open terms. Equating these types makes sense \emph{algebraically} but does follow from the definitions.
