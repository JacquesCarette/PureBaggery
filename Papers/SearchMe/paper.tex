\documentclass[acmsmall,review,prologue,dvipsnames,bookmarks=false]{acmart}
...
\newcommand{\ElE}[1]{\F{\llbracket \black{#1} \rrbracket}_{\UE}}
\newcommand{\EF}[1]{\red{\{\!|}#1\red{|\!\}}}
...
\newcommand{\IN}[2]{#1\mathrel{\blue{\in}}#2}
\newcommand{\IRIN}[2]{#1\mathrel{\green{\in}_{\D{2}}}#2}
\newcommand{\ze}{\C{ze}}
\newcommand{\su}[1]{\C{su}\:#1}
\newcommand{\sndy}[1]{\check{#1}}
\newcommand{\zee}{\sndy{\C{ze}}}
\newcommand{\suu}[1]{\sndy{\C{su}}\:#1}
\newcommand{\LIST}[1]{\D{List}\: #1}
\newcommand{\nil}{\C{[]}}
\newcommand{\cons}[2]{#1\mathop{\red{:\!:}} #2}
\newcommand{\DOL}[1]{\green{\text{\textexclamdown}}#1}

\newcommand{\ONE}{\D{1}}
\newcommand{\void}{\C{\star}}

\newcommand{\SG}[3]{(\V{#1}\hb #2)\pti #3}
\newcommand{\pr}[2]{#1\red{,}\, #2}
\newcommand{\uncurry}[1]{\green{\Uplambda}\:#1}
...
\newcommand{\VERTFPI}[2]{#1 \mathrel{{}_{\UF}\!\!\green{\raisebox{0.03in}[0in][0in]{\ensuremath{\to}}}} #2}
...
\newcommand{\BLEUFPI}[2]{#1 \mathrel{{}_{\UF}\!\!\blue{\raisebox{0.03in}[0in][0in]{\ensuremath{\to}}}} #2}
\newcommand{\VERTFPIB}[2]{#1 \mathrel{{}_{\UF}\!\!\green{\raisebox{0.03in}[0in][0in]{\ensuremath{\leadsto}}}} #2}
\newcommand{\bleuflam}[1]{\F{\uplambda}_{\UF}\:#1}
\newcommand{\bleufapp}[2]{#1 \mathop{\F{\$}_{\UF}} #2}


\newcommand{\VERTEPI}[2]{#1 \mathrel{{}_{\UE}\!\!\green{\raisebox{0.03in}[0in][0in]{\ensuremath{\to}}}} #2}
\newcommand{\vertelam}[1]{\F{\uplambda}_{\UE}\:#1}
\newcommand{\verteapp}[2]{#1 \mathop{\F{\$}_{\UE}} #2}

\newcommand{\BLEUEPI}[2]{#1 \mathrel{{}_{\UE}\!\!\blue{\raisebox{0.03in}[0in][0in]{\ensuremath{\to}}}} #2}
\newcommand{\VERTEPIB}[2]{#1 \mathrel{{}_{\UE}\!\!\green{\raisebox{0.03in}[0in][0in]{\ensuremath{\leadsto}}}} #2}
\newcommand{\bleuelam}[1]{\F{\uplambda}_{\UE}\:#1}
\newcommand{\bleueapp}[2]{#1 \mathop{\F{\$}_{\UE}} #2}

\newcommand{\FSG}[2]{#1 \mathrel{\red{\times_{\UF}}} #2}
\newcommand{\impl}[1]{\ensuremath{\left\{ #1 \right\}}}
\newcommand{\HOLE}{\orange{?}}
\newcommand{\after}{\mathop{\F{\circ}}}
...
\Rule{\V{N}\hab\UF \hg \V{T}\hab\ElF{\V{N}}\fto\SET}
     {\VERTFPI{\V{N}}{\V{T}}\hab \SET}
\qquad
\Rule{\V{f}\hab \PI{x}{\ElF{\V{N}}}{\V{T}\:\V{x}}}
     {\vertflam{\V{f}}\hab \VERTFPI{\V{N}}{\V{T}}}
\qquad
\Rule{\V{k}\hab \VERTFPI{\V{N}}{\V{T}} \hg \V{i}\hab\ElF{\V{N}}}
     {\vertfapp{\V{k}}{\V{i}}\hab \V{T}\:\V{i}}
\]
...
$\ENUM{\C{neutral},\C{combine}}$ and $\ENUM{\C{left},\C{right}}$ encountered above. It would be helpful
if these were not both the \emph{number} $2$. Let us at least promise that $\UF$ will have a sub-universe $\UE$ of finite \remph{tagged} enumerations. Our set-forming braces now become a constructor for $\UF$.
\[
\Axiom{\UE \hab \SET}
\qquad
\Rule{\V{ts} \hab \UE}
...
\Rule{\V{ts} \hab \UE}
     {\EF{\V{ts}} \hab \UF}
\qquad
\ElF{\EF{\V{ts}}} \; = \; \ElE{\V{ts}}
\]
Moreover, to facilitate constructions such as the derivative
...
\Rule{\V{S}\hab\UF \hg \V{T}\hab\ElF{\V{S}}\fto\UF}
     {\FSG{\V{S}}{\V{T}}\hab \UF}
\qquad
\ElF{\FSG{\V{S}}{\V{T}}} = \SG{s}{\ElF{\V{S}}}{\ElF{\V{T}\:\V{s}}}
\]
We promise to return to the definitions of $\UE$ and $\ElE{\blank}$, and their associated notion of tabulated functions, in due course.  However, in choosing the narrative structure for this story, we prefer to wait until we have explored the consequences of our choice to close $\UF$ under dependent pairing. We were taught a lesson!

Our presentation of tabulated functions for dependent pairs is simply currying
\[
\VERTFPI{(\FSG{\V{R}}{\V{S}})}{\V{T}}\; =
\; \VERTFPI{\V{R}}{\LAM{r}{\VERTFPI{\V{S}\:\V{r}}{\LAM{s}{\V{T}\:(\pr{\V{r}}{\V{s}})}}}}
\]
...
\qquad
\vertfapp{\V{k}}{(\pr{\V{r}}{\V{s}})}\; =\; \vertfapp{\vertfapp{\V{k}}{\V{r}}}{\V{s}}
\]
...
\vertfapp{\V{k}}{\impl{\FSG{\V{R}}{\V{S}}}\:(\pr{\V{r}}{\V{s}})} & = & 
  \vertfapp{\vertfapp{\V{k}}{\impl{\V{R}}\:\V{r}}}{\impl{\V{S}\: \V{r}}\:\V{s}}
\end{array}
\]
...
\]
Note that we have move the $\UF$ subscript to the codomain end of the arrow. Abstraction and application are exactly as before. Of course, we could have written $\VERTFPI{\V{N}}{\LAM{n}{\ElF{\V{T}\:\V{n}}}}$ if we were content with a $\SET$, but we are not. The point is to get function spaces \emph{internal} to $\UF$. This rather raises the question of whether we should go further and add codes for function spaces as a constructor for $\UF$. The good news is that we may readily take 
\[ \ElF{\ROUGEPIF{\V{N}}{\V{T}}}\; = \; \VERTFPI{\V{N}}{\LAM{n}{\ElF{\V{T}\:\V{n}}}} \]
...
The price we pay is that $\ElF{\VERTPIF{\V{N}}{\V{T}}}$ is \emph{not} the same type as $\VERTFPI{\V{N}}{\LAM{n}{\ElF{\V{T}\:\V{n}}}}$ when $\V{N}$ and $\V{T}$ are open terms. Equating these types makes sense \remph{algebraically} but does not follow from the definitions. It has the potential, however, to be adopted as a `$\nu$-rule' in the sense of \citet{DBLP:conf/icfp/AllaisMB13}, as it is sufficient to add it only for open terms which have stopped computing by pattern matching.
...
\Rule{\V{C}\hab\FTR\hg \V{X}\hab\SET}
     {\FX{\V{C}}{\V{X}}\hab\SET}
\quad
\FX{\ctr{\V{S}}{\V{P}}}{\V{X}} = \SG{s}{\ElF{\V{S}}}{\VERTFPI{\V{P}\:\V{s}}{\KON{\V{X}}}}
\]
...
\Rule{\V{s}\hab\ElF{\CSh{\V{C}}} \quad
      \V{f}\hab\PI{p}{\ElF{\CPo{\V{C}}{\V{s}}}}{\REC{(\vertfapp{\V{k}}{\V{p}})}}}
     {\nd{\pr{\V{s}}{\V{f}}} \hab \REC{\nd{\pr{\V{s}}{\V{k}}}}}
     \]
%% TODO? SAY MORE ABOUT k?
...
\[
\V{k} \hab \VERTFPI{\CPo{\V{C}}{\V{s}}}{\KON{(\FM{\V{C}}{\V{X}})}} \quad 
\vdash \quad
...
\[
\Rule{\V{P}\hab\UF\quad \V{k} \hab \VERTFPI{\V{P}}{\KON{(\FM{\V{C}}{\V{X}})}}
...
\quad\text{where}\quad
\V{k} \hab 
\VERTFPI{\V{R}}{\LAM{r}{\VERTFPI{\V{S}\:\V{r}}{\KON{(\FM{\V{C}}{\V{X}})}}}}
\]
...
\newcommand{\branch}[3]{#2\mathop{{}_{#1}\hspace*{-0.04in}\raisebox{0.03in}[0in][0in]{\ensuremath{\C{\Yleft}}}}#3}
\newcommand{\branchF}{\branch{\UF}}
\newcommand{\branchE}{\branch{\UE}}
For our $\D{Problem}$, we should start with a base case which shows where $\rec{\blank}$ comes in.
\[\rec{\V{t}} \;=\; \F{solve}\:\basecase\: \V{t}
\qquad\qquad
...
\end{array}
\]
Then our step case, $\branchF{\blank}{\blank}$, should allow us to branch problems over a finite set.
\[
\Rule{\V{S}\hab\UF \quad \V{P}\hab \ElF{\V{S}}\fto\D{Problem}}
     {\branchF{\V{S}}{\V{P}} \hab \D{Problem}}
\quad
\begin{array}[c]{l}
\F{Input}\: (\branchF{\V{S}}{\V{P}}) \;=\; \VERTFPI{\V{S}}{\LAM{s}{\F{Input}\:(\V{P}\:\V{s})}}
\\
\F{Output}\: (\branchF{\V{S}}{\V{P}})\: \V{k} \;=\; 
...
\F{solve}\:\basecase\:\nd{\pr{\V{s}}{\V{k}}} & = &
\nd{\pr{\V{s}}{\F{solve}\:(\branchF{\CPo{\V{C}}{\V{s}}}{\KON{\basecase}})\:\V{k}}} \\
\F{solve}\:(\branchF{(\FSG{\V{R}}{\V{S}})}{\V{P}})\:\V{k}
  \:(\pr{\V{r}}{\V{s}})& = &
\F{solve}\:
(\branchF{\V{R}}{\LAM{r}{
  \branchF{\V{S}\:\V{r}}{\LAM{s}{\V{P}\:(\pr{\V{r}}{\V{s}})}}}})
\:\V{k}\:\V{r}\:\V{s}\\
\vdots\\
\end{array}\]
Of course, we shall need to spell out our story about enumerations to see how the mutual recursion returns from $\branchF{\blank}{\blank}$ problems to $\basecase$ problems, but the good news is that currying out the problem descriptions syncs neatly with the currying out of the type of $\V{k}$ and the story so far typechecks. The bad news, however, is that the problem acquires a larger sequence of smaller branchings while $\V{k}$ stays put, so syntactic guard checkers are fooled into rejecting this entirely sensible definition.


\section{Bleu et Vert: Tabulated Functions Mille-Feuille}
...
We can arrest computation and make simplification explicit by discarding our green-typeset tabulated function space $\VERTFPI{\V{N}}{\V{P}}$ and reintroducing the notion \remph{inductively}, with a canonical (therefore, typeset in blue) type constructor and an explicit value constructor.
\[
\Rule{\V{N}\hab\UF \hg \V{T}\hab\ElF{\V{N}}\fto\SET}
     {\BLEUFPI{\V{N}}{\V{T}}\hab \SET}
\qquad
\Rule{\V{k}\hab\VERTFPIB{\V{N}}{\V{T}}}
     {\nd{\V{k}}\hab\BLEUFPI{\V{N}}{\V{T}}}
\]
But what is being wrapped by $\nd{\blank}$? Exactly a \remph{small-step} version of our computed tabulation!
\[
\Rule{\V{N}\hab\UF \hg \V{T}\hab\ElF{\V{N}}\fto\SET}
     {\VERTFPIB{\V{N}}{\V{T}}\hab \SET}
\qquad     
\VERTFPIB{(\FSG{\V{R}}{\V{S}})}{\V{T}} \;=\;
\BLEUFPI{\V{R}}{\LAM{r}{\BLEUFPI{\V{S}\:\V{r}}{\LAM{s}{\V{T}\:(\pr{\V{r}}{\V{s}})}}}}
\]
...
\[
\Rule{\V{f}\hab \PI{x}{\ElF{\V{N}}}{\V{T}\:\V{x}}}
     {\bleuflam{\V{f}}\hab \BLEUFPI{\V{N}}{\V{T}}}
\qquad
\Rule{\V{k}\hab \BLEUFPI{\V{N}}{\V{T}} \hg \V{i}\hab\ElF{\V{N}}}
     {\bleufapp{\V{k}}{\V{i}}\hab \V{T}\:\V{i}}
\]
...
\bleufapp{\nd{\V{k}}}{\impl{\FSG{\V{R}}{\V{S}}}\:(\pr{\V{r}}{\V{s}})} & = & 
  \bleufapp{\bleufapp{\V{k}}{\V{r}}}{\V{s}}
\end{array}
\]
...
\[
\F{Input}\: (\branchF{\V{S}}{\V{P}}) \;=\; \BLEUFPI{\V{S}}{\LAM{s}{\F{Input}\:(\V{P}\:\V{s})}}
\]
giving us the layers we need to be seen to strip away. We obtain
\[
\F{solve}\:(\branchF{(\FSG{\V{R}}{\V{S}})}{\V{P}})\:\nd{\V{k}}  \;=\;
\F{solve}\:
(\branchF{\V{R}}{\LAM{r}{
  \branchF{\V{S}\:\V{r}}{\LAM{s}{\V{P}\:(\pr{\V{r}}{\V{s}})}}}})
\:\V{k}
\]
which clearly makes the tabulated function smaller.

Now that we have figured out how to cope with the combination of dependent pairs and tabulated function by currying, we should return to $\UF$'s base cases---the finite tagged enumerations we envisaged earlier.

\section{Finite Tagged Enumerations}

Alphabetical order exists, so we \remph{could} choose to represent finite tagged enumerations as strictly ordered lists of strings with elements being strings with an irrelevant proof of membership. However, our misspent youth as C programmers counsels a more primitive approach: finite enumerations are effectively initial segments of the natural numbers, with tags as an aid to legibility. Accordingly we propose to take $\UE = \LIST{\D{String}}$ equipped with a proof-relevant notion of membership amounting to an index.
\[
\Rule{\V{x}\hab \V{X} \quad \V{xs} \hab \D{List}\:\V{X}}
     {\IN{\V{x}}{\V{xs}} \hab \SET}
\qquad
\Axiom{\ze \hab \IN{\V{x}}{\cons{\V{x}}{\V{xs}}}}
\qquad
\Rule{\V{i} \hab \IN{\V{x}}{\V{xs}}}
     {\su{\V{i}} \hab \IN{\V{x}}{\cons{\V{y}}{\V{xs}}}}
\qquad\quad
\begin{array}[c]{l}
\UE \;=\; \LIST{\D{String}} \\
\ElE{\V{ts}} \;=\; \SG{t}{\D{String}}{\IN{\V{t}}{\V{ts}}}
\end{array}
\]

We choose to draw a veil over exactly what a $\D{String}$ is beyond assuming a decidable equality allowing us to define the Boolean membership test, $\IRIN{\V{t}}{\V{ts}}$. It is then straightforward to define an operation $\DOL{\blank}$ which recovers the first index of a string in a list given an implicit and irrelevant proof that it occurs at all. Provided that we are sensible enough not to repeat ourselves, we acquire a convenient tag-based notation which works in all concrete examples. When programming \remph{generically}, however, we leave the strings to unification and work with the numbers. Indeed, we make use of the following, both as expressions and as \remph{patterns}:
\[
\Axiom{\zee\hab\ElE{\cons{\V{t}}{\V{ts}}}} \qquad   \zee \;=\; \pr{\_}{\ze}
\qquad\qquad
\Rule{\V{i}\hab\ElE{\V{ts}}}
     {\suu{\V{i}}\hab\ElE{\cons{\V{t}}{\V{ts}}}} \qquad
  \suu{(\pr{\_}{\V{j}})} \;=\; \pr{\_}{\su{\V{j}}}
\]

Of course, we must also explain how finite tagged enumerations behave as the domains of tabulated functions. We give a `bleu et vert' presentation, following exactly the structure we arrived at, above.
\[
\VERTFPIB{\EF{\V{ts}}}{\V{T}} \;=\; \BLEUEPI{\V{ts}}{\V{T}}
\qquad
\Rule{\V{ts}\hab\UE \hg \V{T}\hab\ElE{\V{ts}}\fto\SET}
     {\BLEUEPI{\V{ts}}{\V{T}}\hab \SET}
\qquad
\Rule{\V{k}\hab\VERTEPIB{\V{ts}}{\V{T}}}
     {\nd{\V{k}}\hab\BLEUEPI{\V{ts}}{\V{T}}}
\]
Again, the `vert' function space computes by case analysis:
\[
\Rule{\V{ts}\hab\UE \hg \V{T}\hab\ElE{\V{ts}}\fto\SET}
     {\VERTEPIB{\V{ts}}{\V{T}}\hab \SET}
\qquad
\begin{array}[c]{l@{\:}lcl@{\:}c@{\:}l}
\VERTEPIB{\nil&}{\V{T}}  & = & & \ONE \\
\VERTEPIB{(\cons{\V{t}}{\V{ts}})&}{\V{T}}  & = & \V{T}\:\zee &\pti& (\BLEUEPI{\V{ts}}{(\V{T}\after\suu{\blank})})\\
\end{array}
\]
We shall, of course, need the appropriate abstraction and application.
\[
\bleuflam{\impl{\EF{\V{ts}}}\:\V{f}} \; =\;
  \nd{\bleuelam{\V{f}}}
\qquad
\Rule{\V{f}\hab \PI{x}{\ElE{\V{ts}}}{\V{T}\:\V{x}}}
     {\bleuelam{\V{f}}\hab \BLEUEPI{\V{ts}}{\V{T}}}
\qquad
\begin{array}[c]{lcl}
\bleuelam{\impl{\nil}\:\V{f}} & = & \nd{\void}\\
\bleuelam{\impl{\cons{\V{t}}{\V{ts}}}\:\V{f}} & = &
  \nd{\pr{\V{f}\:\zee}{\bleuelam{(\V{f}\after\suu{\blank})}}}\\
\end{array}
\]
The abstraction is the classic `vector tabulation' homework: the key insight, tricky for those trained on `for'-loops, is to shift the \remph{function} by composition with successor. The application need only be defined for nonempty enumerations, and is a straightforward vector projection.
\[
\bleufapp{\nd{\V{k}}}{\impl{\EF{\V{ts}}}\:\V{i}} \; = \; 
  \bleueapp{\V{k}}{\V{i}}
\qquad
\Rule{\V{k}\hab \BLEUEPI{\V{ts}}{\V{T}} \hg \V{i}\hab\ElE{\V{ts}}}
     {\bleueapp{\V{k}}{\V{i}}\hab \V{T}\:\V{i}}
\qquad
\begin{array}[c]{lcl}     
\bleueapp{\nd{\pr{\V{z}}{\V{k}}}}{\zee} & = & 
  \V{z} \\
\bleueapp{\nd{\pr{\V{z}}{\V{k}}}}{\suu{\V{i}}} & = & 
  \bleueapp{\V{k}}{\V{i}} \\
\end{array}
\]

\paragraph{The Recursive View from Above}
Now that our $\UF$ universe has a base case, let us close the loop for our recursive view.
We must first extend our notion of $\D{Problem}$ to allow branching over finite tagged enumerations.
\[
\Rule{\V{ts}\hab\UE \quad \V{P}\hab \ElE{\V{ts}}\fto\D{Problem}}
     {\branchE{\V{ts}}{\V{P}} \hab \D{Problem}}
\quad
\begin{array}[c]{l}
\F{Input}\: (\branchE{\V{ts}}{\V{P}}) \;=\; \VERTEPI{\V{ts}}{\LAM{i}{\F{Input}\:(\V{P}\:\V{i})}}
\\
\F{Output}\: (\branchE{\V{ts}}{\V{P}})\: \V{k} \;=\; 
    \PI{i}{\ElE{\V{ts}}}{\F{Output}\:(\V{P}\:\V{i})\:(\verteapp{\V{k}}{\V{i}})}
\end{array}
\]
We must then complete the definition of $\F{solve}$, which we recapitulate in full for your scrutiny.
\[\begin{array}{l@{\:}l@{\:}c@{\:}lcl@{\:}l@{\:}l@{\:}c@{\:}l}
& \D{Problem} & \F{Input} & \multicolumn{4}{l}{\qquad\F{Output}} & \mbox{sub-}\D{Problem} & \F{Input}
\\ \hline
\F{solve}&\basecase &\va{\V{x}} && = & \va{\V{x}}\\
\F{solve}&\basecase &\nd{\pr{\V{s}}{\V{k}}} && = &
\C{\langle}
\pr{\V{s}}{&\F{solve} &(\branchF{\CPo{\V{C}}{\V{s}}}{\KON{\basecase}})&\V{k}&}
\C{\rangle}\smallskip \\
\F{solve}&(\branchF{(\FSG{\V{R}}{\V{S}})}{\V{P}}) &\nd{\V{k}}  & (\pr{\V{r}}{\V{s}})&=&
& \F{solve} &
(\branchF{\V{R}}{\LAM{r}{
  \branchF{\V{S}\:\V{r}}{\LAM{s}{\V{P}\:(\pr{\V{r}}{\V{s}})}}}})
 &\V{k} & \V{r}\:\V{s}\\
\F{solve}&(\branchF{\EF{\V{ts}}}{\V{P}}) &\nd{\V{k}}  &\V{i}&=&
&  \F{solve} &(\branchE{\V{ts}}{\V{P}}) &\V{k} &\V{i}\smallskip\\
\F{solve}&(\branchE{(\cons{\V{t}}{\V{ts}})}{\V{P}}) &\nd{\pr{\V{z}}{\V{k}}} & \zee &=&
&  \F{solve} &(\V{P}\:\zee) &\V{z}\\
\F{solve}&(\branchE{(\cons{\V{t}}{\V{ts}})}{\V{P}}) &\nd{\pr{\V{z}}{\V{k}}} & (\suu{\V{i}}) &=&
&  \F{solve} &(\branchE{\V{ts}}{(\V{P}\after\suu{\blank})}) &\V{k} &\V{i}\\
\end{array}\]
As you can verify by inspection, the $\D{Problem}$ grows and shrinks all over the place, with the type of the $\F{Output}$ varying even in \remph{arity} as it does so. But the $\F{Input}$ gets structurally smaller at every step!
If you are wondering how we ever return to the base case problem, $\basecase$, as found in the first two lines, consider what happens in the penultimate line, where we reach the indicated tag in an enumeration. The subproblem, $\V{P}\:\zee$, thus selected could be further exploration of a larger table, but it could also very well be exactly $\basecase$, if we have finally arrived at a recursive substructure.

\section{Equational Laws for Tabulated Functions}

With all of these pieces now in place, we should check that the laws we hoped for~\ref{sec:fintyp1} hold. Indeed, we can prove that the $\beta$-rule holds for both $\UE$ and $\UF$.
\[
\Rule{\V{f}\hab \PI{x}{\ElE{\V{N}}}{\V{T}\:\V{x}} \hg
      \V{x}\hab \ElE{\V{N}}}
     {\bleueapp{\bleuelam{\V{f}}}{\V{x}}\peq\V{f}\:\V{x}}
\qquad
\Rule{\V{f}\hab \PI{x}{\ElF{\V{S}}}{\V{T}\:\V{x}} \hg
      \V{x}\hab \ElF{\V{S}}}
     {\bleufapp{\bleuflam{\V{f}}}{\V{x}}\peq\V{f}\:\V{x}}
\]
The former holds by induction on the enumeration $\V{N}$; the latter reduces to the former by structural induction on the domain $\V{S}$.

\textbf{TODO: extensionality next, and extract eta as consequence,
  likewise congruence of lambda}
\textbf{TODO: fill in the beta and eta laws and extensionality properties provable for our tabulated functions. They're easy inductions on $\UF$, but we want that stuff established, rather than tripping over it in the middle of something else}.


\section{Maps and Joins that are Awful}

Now that we have justified our recursion view, we can implement a structurally recursive $\F{join}$.

\[ 
\begin{array}[c]{c@{\qquad\qquad}l}
\Rule{\V{ts} \hab \FM{\V{C}}{(\FM{\V{C}}{\V{X}})}}
	{\F{join}\: \V{ts} \hab \FM{\V{C}}{\V{X}}} &
\begin{array}[c]{l@{\;=\;}l}
	\F{join}\: \V{ts} & \F{joinr}\: \V{ts} \: (\rec{\V{ts})}
\end{array} \\
\Rule{\V{ts} \hab \FM{\V{C}}{(\FM{\V{C}}{\V{X}})} \quad \V{r} \hab \REC{\V{ts}}}
	{\F{joinr}\: \V{ts}\: \V{r} \hab \FM{\V{C}}{\V{X}}} &
\begin{array}[c]{l@{\:}c@{\:}c@{\;=\;}l}
	\F{joinr} & \va{\V{x}}     & \va{\V{x}} & \V{x}\\
	\F{joinr} & \nd{\pr{\V{s}}{\V{k}}} & \nd{\pr{\V{s}}{\V{f}}} & \nd{\pr{\V{s}}{\vertflam{\LAM{p}{\F{joinr}\: (\vertfapp{\V{k}}{\V{p}}) \: (\V{f}\: \V{p})}}}} \\
\end{array} \\
\end{array}
\]
Any apparent non-linearity in the equations for $\F{joinr}$ is an informative illusion. Matching on $\V{r}$ leaves no choice as to what $\V{ts}$ must be. Indeed programmers who have enough experience with forced-move unification will confuse their readers by leaving the data themselves implicit and trading openly only in the recursivity proof objects.

As one is wont to do, proving that this $\F{join}$ obeys its laws is next on our plate but we will mostly get a stomach ache from the effort. How are we to reason about $\F{join}$? At the moment we have only its defining equation in terms of $\F{joinr}$ and $\rec{\blank}$.

Naturally, we ought to start with $\F{map}$ first. It too follows the same pattern of definition as $\F{join}$.
\[ 
\begin{array}[c]{c@{\qquad\quad}l}
\Rule{\V{f} \hab \V{X}\fto\V{Y} \quad \V{t} \hab \FM{\V{C}}{\V{X}}}
	{\F{map}\: \V{f}\: \V{t} \hab \FM{\V{C}}{\V{Y}}} &
\begin{array}[c]{l@{\;=\;}l}
	\F{map}\: \V{f}\: \V{t} & \F{mapr}\: \V{f}\:\V{t} \: (\rec{\V{t})}
\end{array} \\
\Rule{\V{f} \hab \V{X}\fto\V{Y} \quad \V{t} \hab \FM{\V{C}}{\V{X}} \quad \V{r} \hab \REC{\V{t}}}
	{\F{mapr}\: \V{f}\: \V{t}\: \V{r} \hab \FM{\V{C}}{\V{Y}}} &
\begin{array}[c]{l@{\:}l@{\:}c@{\:}c@{\;=\;}l}
	\F{mapr} & \V{f} & \va{\V{x}}     & \va{\V{x}} & \va{\V{f}\:\V{x}}\\
	\F{mapr} & \V{f} & \nd{\pr{\V{s}}{\V{k}}} & \nd{\pr{\V{s}}{\V{f}}} & \nd{\pr{\V{s}}{\vertflam{\LAM{p}{\F{mapr}\: \V{f} (\vertfapp{\V{k}}{\V{p}}) \: (\V{f}\: \V{p})}}}} \\
\end{array} \\
\end{array}
\]

But $\F{map}$ and $\F{join}$ should satisfy functor and monad laws, respectively: how are we to \remph{prove} them? We have already made two key strategic blunders which will make our task unhelpfully difficult. The first is simply that we defined $\F{map}$ and $\F{join}$ separately, when we could have extracted both as special cases of monadic `bind'. The second is more subtle. Let us begin by stating an example of the sort of law we want --- $\F{map}$ respects composition.
\[
  \F{map}\:\V{f} ; \F{map}\:\V{g}  \peq \F{map}\:(\V{f};\V{g})
\]
Whether or not our host type theory is blessed with function extensionality, the proof will, of necessity, be pointwise, and it makes sense in any case to make the statement more flexible by considering not only the composite $\V{f};\V{g}$ but any $\V{h}$ with the same behaviour. So
\[
(\forall \V{x}.\; \V{g}\:(\V{f}\:\V{x}) \peq \V{h}\:\V{x}) \; \fto\;
(\forall \V{t}.\; \F{map}\:\V{g}\:(\F{map}\:\V{f}\:\V{t}) \peq \F{map}\:\V{h}\:\V{t})
\]
If we try to prove this by induction on $\V{t}$, we will find that the indirection via $\F{mapr}$ forces us to reason operationally about $\rec{\V{t}}$, and before too long, we shall face the generalisation to $\F{solve}$ by which we defined $\F{rec}$. We should, at the very least, exploit the fact that we have already established the validity of the induction principle for $\V{t}$ which the definitions themselves use. That is to say, we should rather do induction on the \remph{recursivity} of $\V{t}$!
\[
(\forall \V{x}.\; \V{g}\:(\V{f}\:\V{x}) \peq \V{h}\:\V{x}) \; \fto\;
(\forall \V{t}.\;\forall \V{r}\hb\REC{\V{t}},\V{s}\hb\REC{(\F{mapr}\:\V{f}\:\V{t}\:\V{r})}.\; \F{mapr}\:\V{g}\:(\F{mapr}\:\V{f}\:\V{t}\:\V{r})\:\V{s} \peq \F{map}\:\V{h}\:\V{t}\:\V{r})
\]
Note that we are forced to consider the recursivity of the \remph{output} of the inner $\F{mapr}\:\V{f}$ call, in order to make progress on the outer $\F{mapr}\:\V{g}$ call. Are we having fun yet?

Induction on $\V{r}$, rather than $\V{t}$ gives us a base case which follows directly from our elementwise assumption. We also obtain a step case where the shapes match on the nose, and we have induction hypotheses which pertain to a tabulated function $\V{k}$ of substructures and \remph{specific} recursivity witnesse, $\V{kr}$ and $\V{ks}$ in each position.
\[\begin{array}{rl}
&
\vertflam{\LAM{p}{\F{mapr}\:\V{g}
    \:(\vertfapp{(\vertflam{\LAM{p}{\F{mapr}\:\V{f}\:(\vertfapp{\V{k}}{\V{p}})\:(\V{kr}\:\V{p})}})}{\V{p}})
      \:(\V{ks}\:\V{p})}}
\\ \peq &
\vertflam{\LAM{p}{\F{mapr}\:\V{h}\:(\vertfapp{\V{k}}{\V{p}})\:(\V{kr}\:\V{p})}}
\end{array}\]
Firstly, to go under those uses of $\vertflam{\LAM{p}{\cdots}}$, we shall need the \remph{extensionality} property that pointwise equal functions have intensionally equal tabulations: that, we have not yet shown, but it is certainly to be expected. Secondly, we are one tabulated $\beta$-reduction away from a subgoal amenable to our induction hypothesis, so we must prove that $\beta$-law, an obligation we anticipated, above. The problem which then arises is that the recursivity witness $\V{ks}\:\V{p}$ pertains specifically to the $\beta$-redex prior to reduction, so its type will also need repair. This overspecificity to recursivity witnesses causes bureaucratic nightmares which are not insurmountable, but well worth evading.

And before anyone suggests that we might make recursivity witnesses definitionally proof irrelevant, it has long been established that to do so leaves one unable to lean on strong normalisation~\cite{DBLP:conf/types/BradyMM03}. When computing with \remph{open} terms, it is never enough that there is at most one \remph{canonical} proof of recursivity, because there may be bogus but noncanonical proofs to tempt us into trouble.

In some respects, however, our second blunder is rather more basic. We have been trying to reason about the \remph{implementations} of $\F{map}$ and $\F{join}$, rather than working with respect to their \remph{specifications}. The role of recursivity should be only to show those specifications can be made effective.


% TODO
% either cause 'rec' to reduce, and then get ourselves into the same mess that warranted 'solve', OR
% we abstract recursivity proofs and end up in trouble because such proofs can't be proof-irrelevant (safely).

\section{Maps and Joins that are Lawful}

% Just define the relational version!
